...
class Identity(db.Model):
	...
	def __init__(self,openid,group=1,gender = 2,pzn_sessn=0,max_sessn=12,putin_data = '',session_key = ''):
	...
	def create_id_unique_record(cls,openid,uploaded_data,gender=2,pzn_sessn=0,max_sessn=12):
		...
		if exst_record:
			uploaded_data = uploaded_data + '<logged_>Relogin'
			return (exst_record,0)# set a flag for session.add to be skipped in Main.py
